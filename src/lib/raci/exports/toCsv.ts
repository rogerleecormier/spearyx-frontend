...
export async function exportToCsv(
  ...
  state.tasks.forEach(task => {
    ...
    state.roles.forEach(role => {
      ...
      const escapedValue = cellValueStr.includes(',') || cellValueStr.includes('"') || cellValueStr.includes('\n')
 ...
 */
export function downloadCsv(state: RaciState, options: ExportOptions = {}): void {
  exportToCsv(state, options).then(blob => {
