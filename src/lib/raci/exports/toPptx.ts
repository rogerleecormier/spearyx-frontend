...
async function addTableWithOverflowHandling(
  ...
  slide.addTable(tableData, {
    ...
    colW: Array(tableData[0].length).fill(
      (slideWidth - 2 * margin) / tableData[0].length
    ),
  });

  // Add legend
  await addLegendToSlide(
    slide,
    startY + tableHeight + 0.2,
    margin,
    slideHeight
  );
}
...
export async function exportToPptx(
  ...
  try {
    ...
    const headerRow = [
      {
        text: 'Task',
        options: { fill: '4A5568', color: 'FFFFFF', fontSize: 9 },
      },
    ...
    state.tasks?.forEach((task, taskIndex: number) => {
    ...
    await addTableWithOverflowHandling(
      slide,
      tableData,
      currentY,
      margin,
    ...
    throw new Error(
      'Failed to create PowerPoint presentation. Please try again.'
    );
  }
}
...
export function downloadPptx(
  ...
  if (typeof document === 'undefined' || typeof window === 'undefined') {
    throw new Error('downloadPptx can only be called in a browser environment');
  }

  exportToPptx(state, options)
    .then((blob) => {
