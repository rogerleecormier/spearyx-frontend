...
export const RaciValueSchema = z.object({
  R: z.boolean(),
  A: z.boolean(),
  C: z.boolean(),
  I: z.boolean(),
...
export function validateRaciState(state: RaciState): ValidationError[] {
  ...
  const duplicateTasks = taskNames.filter((name, index) => taskNames.indexOf(name) !== index);
  ...
  state.tasks.forEach(task => {
    ...
    const hasAccountable = Object.values(taskMatrix).some(raciValue => raciValue.A);
    if (!hasAccountable) {
      errors.push({
...
export function validateLogoFile(file: File): { valid: boolean; error?: string } {
  ...
  if (!allowedTypes.includes(file.type)) {
    return { valid: false, error: 'Logo must be PNG, SVG, WEBP, or JPEG format' };
  }
